import { ListNature, ListType } from "../types/despType";



export function get_desp(
  nature: ListNature,
  type: ListType,
  pressure: number,
  volume: number,
  ...
  // Constante string
  const ART_4_3: string = "Art 4§3";
  const CAT_I: string = "Cat I";
  const CAT_II: string = "Cat II";
  const CAT_III: string = "Cat III";
  const CAT_IV: string = "Cat IV";
    ...
    if (danger == 1) {
      if (nature === ListNature.LIQUIDE) {
        ...
        if (type === ListType.TUYAUTERIE) {
          ...
          if (pressure >= 35 && pressure < 40 && diamNom < 100 && pvn >= 1000)
            result = CAT_II;
          if (pressure <= 10 && diamNom >= 100 && diamNom < 350)
            result = CAT_II;
      ...
      if (nature === ListNature.LIQUIDE) {
        if (type === ListType.TUYAUTERIE) {
          ...
          if (pressure >= 10 && pressure < 500 && volume >= 1000)
            result = CAT_I;
        ...
        if (type === ListType.TUYAUTERIE) {
          ...
          if (pressure >= 35 && diamNom >= 100 && diamNom < 250)
            result = CAT_II;
          ...
          if (pressure >= 1000 && pressure < 3000 && volume <= 1)
            result = CAT_III;
